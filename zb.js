...
const fromString = (function() {
  ...
  [
    ...
    eval(s)
      ...
      .map((c, i) => {
        ...
        if (!char.hasOwnProperty(c) || char[c].length > _.length) {
          char[c] = _;
        }
      })
  );
  ...
  for (let i = 10; i < 36; i++) {
    ...
    if (!char.hasOwnProperty(c)) {
      // i['toString'](36)
      char[c] = `(${fromNumber(i)})[${transform("toString")}](${fromNumber(
        36
      )})`;
    }
  }
  ...
  for (let i = 0; i < 256; i++) {
    ...
    char[c] = `[][${transform("sort")}][${transform(
      "constructor"
    )}](${transform("return unescape")})()('%'+(${transform(hexStr)}))`;
  }
  ...
  return function(str) {
    ...
    const hexCodes = str
      ...
      .map(code => `('%'+${code})`)
      .join("+");
    return `[][${transform("sort")}][${transform("constructor")}](${transform(
      "return unescape"
    )})()(${hexCodes})`;
  };
})();
...
if (program.runtime) {
  console.log(
    `[][${fromString("sort")}][${fromString("constructor")}](${fromString(
      content
    )})()`
  );
} else if (content) {
